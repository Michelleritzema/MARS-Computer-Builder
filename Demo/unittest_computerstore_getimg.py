...
import requests
import unittest
from CrawlerComputerstore import get_img
...
class TestUM(unittest.TestCase):
    ...
    def test_img(self):
        for item_container in soup.findAll('div', {'class': 'product_container'}):
            self.assertIsNotNone(get_img("product_container", item_container))
        for item_container in soup.findAll('div', {'class': 'product-page'}):
            self.assertIsNotNone(get_img("product-page", item_container))
